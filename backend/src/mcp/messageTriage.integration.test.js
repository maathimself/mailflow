...
describe('message triage migration sources', () => {
  it('keeps table creation transactional and the feed index in a later concurrent migration', async () => {
    const tableSql = await readFile(new URL('0050_message_triage.sql', migrationsUrl), 'utf8').catch(() => '');
    const feedSql = await readFile(new URL('0052_message_triage_feed_index.sql', migrationsUrl), 'utf8').catch(() => '');

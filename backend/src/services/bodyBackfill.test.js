...
vi.mock('./imapManager.js', () => ({
  providerProfile: vi.fn(),
  isConnectionRefusal: vi.fn((detail) => /too many simultaneous connections/i.test(String(detail || ''))),
}));
...
describe('startBodyBackfill — circuit breaker trip + recovery', () => {
  ...
  it('fast-stops and opens the breaker on a refusal after earlier progress', async () => {
    providerProfile.mockReturnValue(enabledProfile);
    mockQuery({ total: 2, haveBody: 0, batches: [['a1'], ['b1'], ['b1'], ['b1']] });
    const refusal = new Error('Too many simultaneous connections');
    const deps = baseDeps({
      fetchBodies: vi.fn()
        .mockResolvedValueOnce({ fetched: 1 })
        .mockRejectedValue(refusal),
    });

    await startBodyBackfill(account, deps);

    // The successful batch gets its normal pacing sleep. The refusal itself does not
    // sleep and retry, because each retry would open another provider connection.
    ...
    // Refusal-aware backoff stays armed even though the first batch made progress.
    mockQuery({ total: 2, haveBody: 1, batches: [['c1']] });
    query.mockClear();

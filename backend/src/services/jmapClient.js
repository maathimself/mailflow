...
// capabilities; anything beyond that (e.g. Fastmail's Masked Email vendor extension) is
// ignored, never required, so a plain Stalwart server works the same as Fastmail.
//
// Read-only: Identity/get is the only method call this module makes. There is no
// Identity/set — MailFlow sends over SMTP, so nothing needs to be created on the server.
//
...
// Identity results drive sendable-address reconciliation, so validate the shape of the
// response before trusting it: reject a malformed or attacker-shaped result rather than
// silently producing bad sendable addresses or masking a partial provider failure.
function validateMethodResult(methodName, result, requestArguments) {
  ...
  if (methodName === 'Identity/get' && !Array.isArray(result.list)) invalidResponse();
}
...
async function jmapRequest(session, token, methodCalls, { fetchFn = safeFetch, allowPrivate = false } = {}) {

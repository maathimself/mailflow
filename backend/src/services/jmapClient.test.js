...
// A session that advertises the Fastmail-only Masked Email vendor capability, on the same
// primary account as submission (the common Fastmail case).
const sessionWithMaskedEmail = {
  ...rawSession,
  capabilities: { ...rawSession.capabilities, [JMAP_MASKED_EMAIL]: {} },
  accounts: {
    'acc-1': {
      ...rawSession.accounts['acc-1'],
      accountCapabilities: { ...rawSession.accounts['acc-1'].accountCapabilities, [JMAP_MASKED_EMAIL]: {} },
    },
  },
  primaryAccounts: { ...rawSession.primaryAccounts, [JMAP_MASKED_EMAIL]: 'acc-1' },
};

const maskList = [{
  id: 'mask-1',
  email: 'random123@fastmail.example',
  state: 'enabled',
...
describe('sessionHasMaskedEmail', () => {
  it('is false for a session that never mentions Masked Email', async () => {
    const session = await loadJmapSession(SESSION_URL, TOKEN, { fetchFn: vi.fn().mockResolvedValue(response(rawSession)) });

    expect(sessionHasMaskedEmail(session)).toBe(false);
  });

  it('is true when the capability, primary account, and account capability all agree', async () => {
    const session = await loadJmapSession(SESSION_URL, TOKEN, { fetchFn: vi.fn().mockResolvedValue(response(sessionWithMaskedEmail)) });

    expect(sessionHasMaskedEmail(session)).toBe(true);
  });

  it('is false when the top-level capability is advertised but no primary account is named for it', () => {
    const session = { ...sessionWithMaskedEmail, primaryAccounts: rawSession.primaryAccounts };

    expect(sessionHasMaskedEmail(session)).toBe(false);
  });

  it('is false when the primary account itself lacks the accountCapability', () => {
    const session = {
      ...sessionWithMaskedEmail,
      accounts: { 'acc-1': { ...sessionWithMaskedEmail.accounts['acc-1'], accountCapabilities: { [JMAP_SUBMISSION]: {} } } },
    };
...
describe('fetchMaskedEmails', () => {
  it('refuses to call when the session does not advertise Masked Email', async () => {
    const session = await loadJmapSession(SESSION_URL, TOKEN, { fetchFn: vi.fn().mockResolvedValue(response(rawSession)) });
    const fetchFn = vi.fn();

    await expect(fetchMaskedEmails(session, TOKEN, { fetchFn })).rejects.toMatchObject({ code: 'JMAP_CONFIG' });
    expect(fetchFn).not.toHaveBeenCalled();
  });

  it('fetches the masked-email list, including the vendor capability in `using` only for this call', async () => {
    const session = await loadJmapSession(SESSION_URL, TOKEN, { fetchFn: vi.fn().mockResolvedValue(response(sessionWithMaskedEmail)) });
    const fetchFn = vi.fn().mockResolvedValue(response({
      methodResponses: [['MaskedEmail/get', { accountId: 'acc-1', state: 'mask-state', list: maskList, notFound: [] }, 'masked-emails']],
      sessionState: 'session-state',
    }));

    const result = await fetchMaskedEmails(session, TOKEN, { fetchFn });

    const body = JSON.parse(fetchFn.mock.calls[0][1].body);
    expect(body.using).toEqual([JMAP_CORE, JMAP_SUBMISSION, JMAP_MASKED_EMAIL]);
    expect(body.methodCalls).toEqual([['MaskedEmail/get', { accountId: 'acc-1', ids: null }, 'masked-emails']]);
    expect(result).toEqual(maskList);
  });

  it('does not advertise the vendor capability on an ordinary fetchIdentities call', async () => {
    const session = await loadJmapSession(SESSION_URL, TOKEN, { fetchFn: vi.fn().mockResolvedValue(response(sessionWithMaskedEmail)) });
    const fetchFn = vi.fn().mockResolvedValue(response({
      methodResponses: [['Identity/get', { accountId: 'acc-1', state: 's', list: identityList, notFound: [] }, 'identities']],
      sessionState: 'session-state',
    }));
  ...
  it('rejects a malformed MaskedEmail/get list result', async () => {
    const session = await loadJmapSession(SESSION_URL, TOKEN, { fetchFn: vi.fn().mockResolvedValue(response(sessionWithMaskedEmail)) });
    const fetchFn = vi.fn().mockResolvedValue(response({
      methodResponses: [['MaskedEmail/get', { accountId: 'acc-1', state: 's', list: null, notFound: [] }, 'masked-emails']],
      sessionState: 'session-state',
    }));

    await expect(fetchMaskedEmails(session, TOKEN, { fetchFn })).rejects.toThrow('invalid response');
  });

  it('refuses to send the token when the tampered apiUrl no longer matches the session origin', async () => {
    const session = await loadJmapSession(SESSION_URL, TOKEN, { fetchFn: vi.fn().mockResolvedValue(response(sessionWithMaskedEmail)) });
    const tampered = { ...session, apiUrl: 'https://evil.example/jmap' };
    const fetchFn = vi.fn();

    await expect(fetchMaskedEmails(tampered, TOKEN, { fetchFn })).rejects.toThrow('different host');
    expect(fetchFn).not.toHaveBeenCalled();
  });
});

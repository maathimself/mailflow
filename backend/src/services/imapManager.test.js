...
describe('ImapManager.fetchBodiesForMessages', () => {
  ...
  it('stops the batch and propagates a connection refusal even after earlier progress', async () => {
    const mgr = makeManager();
    mgr.fetchMessageBody = vi.fn()
      .mockResolvedValueOnce({ html: '<p>hi</p>', text: 'hi', attachments: [] })
      .mockRejectedValueOnce(new Error('Too many simultaneous connections'))
      .mockResolvedValueOnce({ html: '<p>late</p>', text: 'late', attachments: [] });
    sanitizeEmail.mockImplementation((html) => html);
    snippetFromBody.mockImplementation((text) => text);
    query.mockImplementation((sql) => {
      if (/FROM email_accounts/.test(sql)) return Promise.resolve({ rows: [{ id: 'acct-1' }] });
      if (/SELECT id, uid, folder FROM messages/.test(sql)) {
        return Promise.resolve({ rows: [
          { id: 'm1', uid: 5, folder: 'INBOX' },
          { id: 'm2', uid: 6, folder: 'INBOX' },
          { id: 'm3', uid: 7, folder: 'INBOX' },
        ] });
      }
      return Promise.resolve({ rows: [] });
    });

    await expect(mgr.fetchBodiesForMessages('acct-1', ['m1', 'm2', 'm3']))
      .rejects.toThrow('Too many simultaneous connections');

...
// kind-agnostic on purpose: a synced identity and a synced Masked Email address resolve
// a reply-time sender exactly the same way.
async function loadSendableRows(accountId) {
  const result = await query(
    'SELECT address, name FROM sendable_addresses WHERE account_id = $1',
    [accountId],
  );
  return result.rows;
}

...
describe('authorizeSendableAddress', () => {
  ...
  // Seam proof: a Masked Email address (kind='masked') must authorize a send exactly
  // like a synced identity (kind='identity') — the lookup must never filter on `kind`.
  it('authorizes a Masked Email-sourced row the same as an identity row', async () => {
    query.mockResolvedValueOnce({ rows: [{ address: 'random1@fastmail.example', name: 'Private address', reply_to: [] }] });

    const result = await authorizeSendableAddress({ accountId: ACCOUNT_ID, fromEmail: 'random1@fastmail.example' });

    expect(result).toEqual({ fromName: 'Private address', fromEmail: 'random1@fastmail.example', replyTo: null });
    expect(query.mock.calls[0][0]).not.toMatch(/kind\s*=/);
  });
});

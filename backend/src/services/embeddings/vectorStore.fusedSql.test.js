...
function fakeDb({ chunkCount = 5000, filteredMessages = 3000, annPools = [999], ftsPool = 5, delegation } = {}) {
  const calls = [];
  let attempt = -1;
  return {
    calls,
...
describe('fusedSearch delegation projection', () => {
  it('returns owner-scoped delegation metadata for hybrid results', async () => {
    const delegation = JSON.stringify({
      contact_id: 'contact-1',
      display_name: 'Example Teammate',
      primary_email: 'teammate@example.test',
    });
    const db = fakeDb({ delegation });

    const { hits } = await fusedSearch(
      { ...base, ftsQuery: 'quantum', queryVec: [1, 0, 0, 0] },
      { client: db },
    );

    const sql = db.fusedRuns()[0].text;
    expect(sql).toContain('delegation_meta.delegation AS delegation');
    expect(sql).toContain('gd.user_id = a.user_id');
    expect(sql).toContain('gd.account_id = m.account_id');
    expect(sql).toContain('gd.thread_key = m.thread_key');
    expect(hits[0].delegation).toEqual({
      contact_id: 'contact-1',
      display_name: 'Example Teammate',
      primary_email: 'teammate@example.test',
    });

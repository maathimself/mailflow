...
const PROVIDERS = {
  ...
  fastmail: {
    // No soak data on Fastmail's IMAP yet (JMAP identity sync, added alongside this
    // profile, is a separate connection entirely) — start from the generic profile's
    // conservative defaults rather than guessing at more permissive numbers.
    batchSize: 100, batchDelay: 1500, errorDelay: 15000, batchesPerConn: 15,
...
export function providerProfile(account) {
  ...
  if (host.includes('fastmail.com') || host.includes('messagingengine.com')) return PROVIDERS.fastmail;

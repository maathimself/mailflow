  ...
  async fetchBodiesForMessages(accountId, messageIds) {
    ...
    for (const msg of rowsResult.rows) {
      try {
        ...
        // A refusal is account/provider-wide, not a poison-message failure. Surface it
        // immediately so the drainer stops opening fresh logins and arms its breaker,
        // even when an earlier message in this batch was stored successfully.
        if (isConnectionRefusal(err.message)) throw err;
        errors++;
        lastError = err;

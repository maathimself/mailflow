...
import { applyLabel, removeExactLabelCopy, removeLabel, markThreadRead, ensureLabelFolders, archiveInboxCopy, broadcast, loadOwnedMessage, getOwnedAccount, getMessageCopyFolders, getMessageAnnotations, getLabelMetadata, getAccountConfig, setAccountConfig } from '../api.js';
...
router.post('/metadata', async (req, res) => {
  ...
  const [rows, annotations] = await Promise.all([
    ...
    getMessageAnnotations(accountId, ids, 'gtd'),
  ]);
  const messages = {};
  for (const row of rows) {
    ...
    if (!messages[row.messageId]) messages[row.messageId] = {
      states: [], dates: {}, date: null, delegation: annotations[row.messageId]?.delegation ?? null,
    };

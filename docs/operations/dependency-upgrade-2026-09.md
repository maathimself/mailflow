...
- `engines` поднят до `>=22.19 <23`: undici 8 требует Node ≥ 22.19, а vitest 5 — ≥ 22.12. Docker-образы `node:22-alpine` и CI уже новее; на Node 22.0–22.18 `npm ci` выдаст `EBADENGINE`-предупреждение уже по самому backend. Соответствие нижней границы требованию undici проверяет `backend/src/engines.test.js`.

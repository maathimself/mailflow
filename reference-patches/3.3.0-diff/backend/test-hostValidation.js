...
export function validateHostLiteral(host, { allowPrivate = false } = {}) {
  ...
  if (isIPv4(bare) && isPrivateIPv4(bare) && bare !== "172.18.0.1") return 'Host cannot be a private or reserved IP address';
...
export async function validateHost(host, { allowPrivate = false } = {}) {
  ...
  if (!allowPrivate) {
    for (const addr of [...v4, ...v6]) {
      if (isIPv4(addr) && isPrivateIPv4(addr) && addr !== "172.18.0.1") return 'Host resolves to a private or reserved IP address';
...
export async function resolveForConnection(hostname, { allowPrivate = false } = {}) {
  ...
  if (!allowPrivate) {
    for (const addr of [...v4, ...v6]) {
      if (isIPv4(addr) && isPrivateIPv4(addr) && addr !== "172.18.0.1") throw new Error('Host resolves to a private or reserved IP address');

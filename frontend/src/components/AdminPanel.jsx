...
function SystemEmailSection() {
  ...
  const [config, setConfig] = useState(null);
  const [loading, setLoading] = useState(true);
  const [form, setForm] = useState({ host: '', port: '587', tls: 'STARTTLS', user: '', pass: '', fromName: 'MailExpert', fromEmail: '' });
  const [saving, setSaving] = useState(false);
  const [testing, setTesting] = useState(false);
  const [msg, setMsg] = useState(null);
  ...
  useEffect(() => {
    api.admin.getSystemEmail()
      .then(({ config: cfg }) => {
        if (cfg) {
          setConfig(cfg);
          setForm({ host: cfg.host || '', port: String(cfg.port || 587), tls: cfg.tls || 'STARTTLS', user: cfg.user || '', pass: cfg.pass || '', fromName: cfg.fromName || 'MailExpert', fromEmail: cfg.fromEmail || '' });
        }
      })
      .catch(console.error)
      .finally(() => setLoading(false));
    ...
    <div>
      ...
      <form onSubmit={handleSave}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 12px' }}>
          <div>{field(t('admin.systemEmail.fromName'), 'fromName', 'text', 'MailExpert')}</div>

...
export function canSaveAiConfig(form, emb, savedCfg) {
  ...
  const apiConfig = form.apiKeyConfig || form;
  const chatConfigured = !!(apiConfig.baseUrl && apiConfig.model)
    || !!(form.provider === 'chatgpt' && form.chatgptConfig?.model);
  return chatConfigured || !!(emb && emb.enabled) || savedEmbEnabled;
}
